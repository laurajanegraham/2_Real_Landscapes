...
### Predictor variables 10km

Some of the predictor variables will be at the 10km level, these are temperature, precipitation, proportion of habitat and Shannon diversity of habitat. 
...
lcm_stats <- ldply(preds$grid_ref_levels, function(i) {
  ...
  grid_ref_levels <- i
  return(data.frame(grid_ref_levels = grid_ref_levels, prop = lcm_prop, shannon = lcm_shannon))
})

# combine the climate and land cover predictors 
preds <- merge(preds, lcm_stats)
...
write.csv(preds, file = "data/predictors_10km.csv", row.names = FALSE)
```
